...
// requestRulePatcher adds trace side effects to supported request-dependent
// multipliers without changing the stored expression or its numeric result.
// Only the canonical `(condition ? number : 1)` shape is instrumented. More
// complex expressions continue to run unchanged and simply have no trace.
