...
const { RouterProvider, createMemoryHistory, createRootRoute, createRoute, createRouter } = await import('@tanstack/react-router')
...
function item(id = 1, overrides: Partial<TodoItem> = {}): TodoItem {
  return {
    id: `open_source_bounty:${id}`, source_id: id, category: 'open_source_bounty',
    type: 'comment', title: 'Bounty update', summary: `Notification ${id}`,
    read: false, created_at: 1_786_400_000, updated_at: 1_786_400_000, ...overrides,
  }
}
function page(overrides: Partial<TodoPage> = {}): TodoPage {
  return {
    items: [item()], page: 1, page_size: 50, total: 1, category: 'all',
    unread_count: 1, total_unread_count: 1, unread_by_category: { open_source_bounty: 1 },
    categories: [{ key: 'open_source_bounty', total: 1, unread: 1 }], ...overrides,
  }
}
...
async function renderList() {
  ...
  const rootRoute = createRootRoute({
    component: () => createElement(QueryClientProvider, { client: queryClient },
      createElement(I18nextProvider, { i18n }, createElement(UnifiedTodoList))),
  })
  const indexRoute = createRoute({ getParentRoute: () => rootRoute, path: '/', component: () => null })
  const securityRoute = createRoute({ getParentRoute: () => rootRoute, path: 'security', component: () => null })
  ...
  await act(async () => { root.render(createElement(RouterProvider, { router })); await flush() })
  await act(flush)
  return { container, router, async close() {
...
async function click(container: HTMLElement, label: string) {
  await act(async () => { button(container, label).click(); await flush() })
  await act(flush)
}
beforeEach(() => {
...
describe('todo feed regressions', () => {
  test('fetches later pages and resets pagination when the category changes', async () => {
    ...
    api.get = (async (url: string) => {
      ...
      return response(page({ page: currentPage, total: 51,
  ...
  test('opens the destination before the read receipt resolves and catches failure', async () => {
    const read = deferred()
    api.get = (async () => response(page({ items: [item(1, {
      category: 'security_review', summary: 'Open the audit timeline',
    })] }))) as typeof api.get
    ...
    try {
      ...
      await act(async () => { read.reject(new Error('Read receipt failed')); await flush() })
    ...
    } finally { read.resolve(response({ marked: 1 })); await rendered.close() }
  })

  test('deduplicates a fast double click without locking other unread rows', async () => {
    const first = deferred()
    const ids: number[] = []
    api.get = (async () => response(page({ items: [item(1), item(2)], total: 2 }))) as typeof api.get
    api.post = ((_: string, body: { ids: number[] }) => {
      ids.push(body.ids[0])
      return body.ids[0] === 1 ? first.promise : Promise.resolve(response({ marked: 1 }))
    }) as typeof api.post
    const rendered = await renderList()
    try {
      await act(async () => {
      ...
      await act(async () => { first.resolve(response({ marked: 1 })); await flush() })
    } finally { first.resolve(response({ marked: 1 })); await rendered.close() }
  })

  test('retains existing rows when a background refresh fails', async () => {
    const rendered = await renderList()
    try {
      api.get = (async () => { throw new Error('Temporary outage') }) as typeof api.get
      await click(rendered.container, 'Refresh')
      assert.ok(rendered.container.textContent?.includes('Failed to load to-dos'))
      assert.ok(rendered.container.textContent?.includes('Notification 1'))
      assert.equal(rendered.container.textContent?.includes('No pending to-dos'), false)
    } finally { await rendered.close() }
  })
  ...
  test('returns to the first page when the last page disappears', async () => {
    ...
    api.get = (async (url: string) => {
      gets.push(url)
      return response(gets.length === 1 ? page({ total: 51 }) : page({ items: [], total: 0 }))
    }) as typeof api.get
    const rendered = await renderList()
    try {
      await click(rendered.container, 'Next page')
  ...
  test('renders a notification even when its timestamp is outside the Date range', async () => {
    api.get = (async () => response(page({ items: [item(1, { updated_at: 9e15 })] }))) as typeof api.get
    const rendered = await renderList()
    try {
      assert.ok(rendered.container.textContent?.includes('Notification 1'))
  ...
  for (const change of ['account', 'session', 'role'] as const) {
    test(`does not reuse cached rows or late read errors after a ${change} change`, async () => {
      ...
      try {
        ...
        await act(async () => {
          if (change === 'session') {
            useAuthStore.setState((state) => ({ auth: { ...state.auth, session: {
              sid: 'replacement-session', current: true, login_method: 'password',
              ip: '', user_agent: '', created_at: 1, last_active_at: 1, expires_at: 2,
            } } }))
          } else {
            useAuthStore.getState().auth.setUser({
              id: change === 'account' ? 11 : 10, username: 'replacement', role: change === 'role' ? 1 : 10,
            })
          }
          await flush()
        })
        assert.equal(rendered.container.textContent?.includes('Notification 1'), false)
        await act(async () => {
