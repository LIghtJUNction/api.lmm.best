...
legacy="${MIGRATION_LEGACY_PATH:-$repo_root/apps/api-rust/routes/legacy-go-routes.tsv}"
...
cut -f1-2 <(tsv_without_crlf "$gate") | tail -n +2 |
  diff -u <(tsv_without_crlf "$legacy" | cut -f1-2) -
...
require_frozen_model_delete_501_evidence() {
  ...
  validator="$repo_root/apps/api-rust/behavior-oracle/tests/validate-frozen-model-delete-501-evidence.sh"
...
candidate_dir="$repo_root/apps/api-rust/apps/lmm-api-rs/src/migration_routes"
mapfile -t candidate_files < <(rg --files "$candidate_dir")
candidate_count=${#candidate_files[@]}
candidate_mod="$repo_root/apps/api-rust/apps/lmm-api-rs/src/migration_routes.rs"
...
root_router="${MIGRATION_ROOT_ROUTER_PATH:-$repo_root/apps/api-rust/apps/lmm-api-rs/src/http.rs}"
