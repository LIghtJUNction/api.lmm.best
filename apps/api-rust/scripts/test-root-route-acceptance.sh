...
manifest="$repo_root/apps/api-rust/apps/lmm-api-rs/tests/root-route-acceptance/Cargo.toml"
runner="$repo_root/apps/api-rust/scripts/run-root-route-acceptance.sh"
...
grep -Fq 'legacy-go-routes.tsv' "$repo_root/apps/api-rust/apps/lmm-api-rs/tests/root-route-acceptance/src/inventory.rs" || {
  echo "root-route inventory is not bound to the frozen 356-route baseline" >&2
  exit 1
}
grep -Fq 'migration-plan.tsv' "$repo_root/apps/api-rust/apps/lmm-api-rs/tests/root-route-acceptance/src/inventory.rs" || {
  echo "root-route inventory is not bound to migration auth classes" >&2
  exit 1
}

python3 - "$repo_root/apps/api-rust/apps/lmm-api-rs/src/main.rs" <<'PY'
