...
checker="$repo_root/apps/api-rust/scripts/check-behavior-deviations.sh"
ledger="$repo_root/apps/api-rust/routes/behavior-deviations.tsv"
...
awk -F '\t' 'BEGIN { OFS=FS }
  NR == 1 { print; next }
  !changed { $7="apps/api-rust/behavior-oracle/tests/does-not-exist.sh"; changed=1 }
  { print }
