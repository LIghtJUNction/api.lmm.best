...
write_rust_build_manifest() {
  ...
  (
    cd "$repo_root"
    printf '%s\0' apps/api-rust/Cargo.toml apps/api-rust/Cargo.lock apps/api-rust/apps/lmm-api-rs/Cargo.toml
    [[ ! -f apps/api-rust/apps/lmm-api-rs/build.rs ]] || printf '%s\0' apps/api-rust/apps/lmm-api-rs/build.rs
    for directory in apps/api-rust/apps/lmm-api-rs/src apps/api-rust/apps/lmm-api-rs/assets apps/api-rust/crates; do
...
"$repo_root/apps/api-rust/scripts/capture-legacy-route-contract.sh" --base-url "http://127.0.0.1:$go_port" --request "$runtime/requests/api-token-anonymous.json" --output "$runtime/fixtures/api-token-anonymous.json" --postgres-url "$go_dsn"
GO_BASE_URL="http://127.0.0.1:$go_port" RUST_BASE_URL="http://127.0.0.1:$rust_port" \
  ...
  "$repo_root/apps/api-rust/scripts/run-route-differential.sh" "$runtime/fixtures"
scenario_total=$((scenario_total + 1))
