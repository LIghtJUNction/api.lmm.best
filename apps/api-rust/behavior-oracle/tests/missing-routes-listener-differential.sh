...
while IFS=$'\t' read -r class method frozen_path request_path auth_case body; do
  ...
  "$repo_root/apps/api-rust/scripts/capture-legacy-route-contract.sh" \
...
ROUTE_REQUEST_DIR="$runtime/requests" ROUTE_REQUIRE_EFFECTS=strict \
  "$repo_root/apps/api-rust/scripts/run-route-differential.sh" "$runtime/fixtures" | tee "$runtime/results.jsonl"
