...
checker="$repo_root/apps/api-rust/tests/scripts/check-behavior-deviations.sh"
ledger="$repo_root/apps/api-rust/tests/fixtures/routes/behavior-deviations.tsv"
...
awk -F '\t' 'BEGIN { OFS=FS }
  NR == 1 { print; next }
  !changed { $7="apps/api-rust/tests/behavior-oracle/tests/does-not-exist.sh"; changed=1 }
  { print }
