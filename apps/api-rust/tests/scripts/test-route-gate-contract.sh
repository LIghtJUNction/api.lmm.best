...
validator_run --mode source --gate "$synthetic/migration-gate.tsv" \
...
validator_run --mode activate --gate "$synthetic/migration-gate.tsv" \
  --frozen-contract "$synthetic/frozen-route-auth.tsv" --frozen-contract-sha256 "$synthetic_frozen_sha" \
  --evidence-root "$synthetic" --revision "$revision" \
  --migration-compatibility "$synthetic/migration-compatibility.env"

first_verified=$(sed -n '2p' "$synthetic/migration-gate.tsv")
...
make_route_case() {
  ...
  cp "$single_gate" "$case_dir/migration-gate.tsv"
  ...
  sed "s/$old_sha/$new_sha/" "$case_dir/migration-gate.tsv" >"$case_dir/gate.new"
  mv "$case_dir/gate.new" "$case_dir/migration-gate.tsv"
  expect_fail validator_run --mode source --gate "$case_dir/migration-gate.tsv" \
...
cp "$single_gate" "$duplicate_bytes_dir/migration-gate.tsv"
...
sed "s/$old_compile_sha/$new_compile_sha/" "$duplicate_bytes_dir/migration-gate.tsv" >"$duplicate_bytes_dir/gate.new"
mv "$duplicate_bytes_dir/gate.new" "$duplicate_bytes_dir/migration-gate.tsv"
expect_fail validator_run --mode source --gate "$duplicate_bytes_dir/migration-gate.tsv" \
  --frozen-contract "$frozen" --frozen-contract-sha256 "$frozen_sha" \
...
expect_fail env LMM_ROUTE_GATE_TEST_MODE=1 "$validator" --snapshot-dir "$runtime/changed-frozen-snapshot" \
  ...
  --evidence-root "$repo" --revision "$revision"
expect_fail env MIGRATION_FROZEN_ROUTE_AUTH_PATH="$changed_frozen" \
  "$repo/apps/api-rust/tests/scripts/check-migration-plan.sh"

make_migration_case() {
  local name=$1 file=$2 filter=$3
  local case_dir="$runtime/migration-$name" old_sha new_sha
  mkdir -p "$case_dir"
  cp "$synthetic/"{migration-gate.tsv,frozen-route-auth.tsv,validate-route-gate,migration-compatibility.env} "$case_dir/"
  ...
  expect_fail validator_run --mode activate --gate "$case_dir/migration-gate.tsv" \
...
{
  printf '%s  migration-gate.tsv\n' "$(sha256sum "$replacement/migration-gate.tsv" | awk '{print $1}')"
  printf '%s  validate-route-gate\n' "$(sha256sum "$validator" | awk '{print $1}')"
  printf '%s  migration-compatibility.env\n' \
    "$(sha256sum "$repo/packaging/common/lmm-api/migration-compatibility.env" | awk '{print $1}')"
...
LMM_ROUTE_GATE_TEST_MODE=1 LMM_ROUTE_GATE_TEST_SNAPSHOT_READY_FILE="$ready" \
  ...
  --gate "$replacement/migration-gate.tsv" --frozen-contract "$replacement/frozen-route-auth.tsv" \
...
sed -n '1,2p' "$current_gate" >"$replacement/migration-gate.tsv"
: >"$release"
...
cp "$current_gate" "$manifest_swap/migration-gate.tsv"
...
(
  cd "$manifest_swap"
  sha256sum migration-gate.tsv validate-route-gate migration-compatibility.env frozen-route-auth.tsv >"$manifest_swap_file"
)
...
LMM_ROUTE_GATE_TEST_MODE=1 \
  ...
  --gate "$manifest_swap/migration-gate.tsv" --frozen-contract "$manifest_swap/frozen-route-auth.tsv" \
...
mkdir -p "$asset_swap"
cp "$current_gate" "$asset_swap/migration-gate.tsv"
...
(
  cd "$asset_swap"
  sha256sum migration-gate.tsv validate-route-gate migration-compatibility.env frozen-route-auth.tsv >"$asset_swap_file"
)
...
LMM_ROUTE_GATE_TEST_MODE=1 \
  ...
  --gate "$asset_swap/migration-gate.tsv" --frozen-contract "$asset_swap/frozen-route-auth.tsv" \
