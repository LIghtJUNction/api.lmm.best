...
start_go_listener() {
  ...
  SQLITE_PATH="$runtime/go.db?_busy_timeout=30000" PORT="$go_port" REDIS_CONN_STRING="redis://:$(printf %s "$go_password")@127.0.0.1:$go_valkey_port" SESSION_SECRET='public-differential-synthetic' GLOBAL_API_RATE_LIMIT_ENABLE=false CRITICAL_RATE_LIMIT_ENABLE=false TRUSTED_PROXIES=none GIN_MODE=release "$runtime/legacy-go" >"$runtime/go.log" 2>&1 &
  local child=$!
...
start_go_listener
sqlite3 "$runtime/go.db" "INSERT OR REPLACE INTO options(key,value) VALUES ('Notice','notice value'),('About','about value'),('HomePageContent','home value'),('ExposeRatioEnabled','true'),('ModelRatio','{\"gpt\":1.0}'),('CompletionRatio','{\"gpt\":2.0}'),('CacheRatio','{\"gpt\":0.5}'),('CreateCacheRatio','{\"gpt\":0.25}'),('ModelPrice','{\"gpt\":3.0}');"
stop_owned go_pid
start_go_listener
...
SQL
port_free "$rust_port" || exit 1; DATABASE_URL="postgresql://lmm_public_runtime@127.0.0.1:$pg_port/public_diff" VALKEY_URL="redis://:$(printf %s "$rust_password")@127.0.0.1:$rust_valkey_port" LMM_RS_LISTEN_ADDR="127.0.0.1:$rust_port" LMM_RS_SLOT=blue LMM_SCHEMA_CONTRACT=1 SESSION_SECRET="$rust_session_secret" CRYPTO_SECRET="$rust_crypto_secret" GLOBAL_API_RATE_LIMIT_ENABLE=false CRITICAL_RATE_LIMIT_ENABLE=false VERSION=v0.0.0 "$repo_root/apps/api-rust/target/debug/lmm-api-rs" >"$runtime/rust.log" 2>&1 & record_pid rust_pid "$!"; wait_http "$rust_port" /readyz "$rust_pid"
...
for route in /api/notice /api/about /api/home_page_content /api/user-agreement /api/privacy-policy /api/ratio_config /api/uptime/status; do compare "$route"; done
[[ $scenario_total == 7 ]] || { echo "scenario count mismatch: $scenario_total" >&2; exit 1; }
