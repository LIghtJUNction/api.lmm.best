...
use lmm_api_rs::{
    ...
    protocol_rollout::{
        LocalConversionError, LocalConversionSummary, LocalRequest, ShadowDifference,
        ShadowRunner,
    },
    route_ownership::{
        DifferentialClass, OwnershipDecision, OwnershipEvidence, OwnershipGate,
        RouteOwnershipScope, MIN_REVIEW_CANARY_BASIS_POINTS,
    },
};
use lmm_contracts::relay::{
    CacheUsage, CanonicalContent, ClaudeStreamSemanticEvent, SemanticBillingUsage,
    Envelope, Feature, FinishReason, FunctionData, Item, ItemKind, JsonData, Loss, LossCode,
    Media, MediaKind, Money, OpaqueId, OpaqueIdProvenance, OpaqueProviderState, Part, PartKind,
    Protocol, Provenance, Role, SemanticUsage, Tool, ToolChoice,
    TokenUsage,
    canonical_request_to_claude, canonical_request_to_gemini_for_model,
    canonical_request_to_openai_chat, canonical_response_to_claude,
    ...
    openai_stream_to_canonical, openai_responses_request_to_canonical,
    openai_responses_response_to_canonical, responses_stream_to_canonical,
    ClaudeRequest, ClaudeResponse, ClaudeStreamSnapshot, GeminiRequest, GeminiResponse,
    GeminiStreamSnapshot, OpenAiChatRequest, OpenAiChatResponse, OpenAiStreamSnapshot,
    OpenAiResponsesRequest, ResponsesResponse, ResponsesStreamSnapshot,
};
...
const ARBITRARY_SSE_BYTES: &[u8] = &[
    ...
    0xef, 0xbb, 0xbf, b'e', b'v', b'e', b'n', b't', b':', b'f', b'u', b't', b'u',
    b'r', b'e', b'\r', b'\n', b'd', b'a', b't', b'a', b':', b' ', 0xff, b'\n',
    b'\n', b'r', b'e', b't', b'r', b'y', b':', b' ', b'+', b'1', b'\n', b'\n',
];
...
fn request_conformance_preserves_order_ids_and_provider_signatures() {
    ...
    assert!(chat.value.messages.iter().flat_map(|message| &message.parts).any(
        |part| matches!(part, CanonicalContent::ToolCall { id, .. } if id == "call-chat")
    ));
    assert!(chat.value.messages.iter().any(|message| message.role == lmm_contracts::relay::Role::Tool));
    ...
    let gemini = gemini_request_to_canonical_for_model(gemini, "gemini-2.5-pro")
    ...
    assert!(claude.value.messages.iter().flat_map(|message| &message.parts).any(
...
fn request_round_trip_keeps_authentic_tool_and_signature_data() {
    ...
    let gemini: GeminiRequest = serde_json::from_str(GEMINI_REQUEST).expect("Gemini corpus");
    let canonical = gemini_request_to_canonical_for_model(gemini, "gemini-2.5-pro")
        .expect("Gemini conversion");
    let round_trip = canonical_request_to_gemini_for_model(canonical.value, "gemini-2.5-pro", false)
    ...
    assert_eq!(round_trip.value.contents[0].parts[0].thought_signature.as_deref(), Some("sig-gemini"));
...
fn response_conformance_preserves_usage_finish_order_and_opaque_state() {
    ...
    assert_eq!(chat.value.usage.as_ref().map(|usage| usage.total_tokens), Some(3));
    assert!(matches!(chat.value.output.first(), Some(CanonicalContent::Text { text }) if text == "answer"));
    ...
    let responses = openai_responses_response_to_canonical(responses).expect("Responses response conversion");
    assert_eq!(responses.value.finish_reason, Some(FinishReason::Stop));
    assert_eq!(responses.value.usage.as_ref().map(|usage| usage.total_tokens), Some(3));
    ...
    assert_eq!(claude.value.usage.as_ref().map(|usage| usage.total_tokens), Some(3));
    let claude_wire = canonical_response_to_claude(claude.value).expect("Claude response round trip");
    assert_eq!(claude_wire.value.content[0].signature.as_deref(), Some("sig-claude"));
    ...
    assert_eq!(gemini.value.usage.as_ref().map(|usage| usage.total_tokens), Some(3));
    assert!(gemini.value.output.iter().any(
        |part| matches!(part, CanonicalContent::ProviderState { state } if state.raw == JsonData::String("sig-gemini".to_owned()))
    ));
    let gemini_wire = canonical_response_to_gemini(gemini.value).expect("Gemini response round trip");
    assert_eq!(gemini_wire.value.candidates[0].content.parts[0].thought_signature.as_deref(), Some("sig-gemini"));
}
...
fn usage_cache_billing_differential_matches_legacy_and_new_semantics() {
    ...
    let cases = [
        ("openai-chat", chat.usage.expect("Chat usage"), (2, 1, 3)),
        (
            "openai-responses",
            responses.usage.expect("Responses usage"),
            (2, 1, 3),
        ),
        ("claude", claude.usage.expect("Claude usage"), (2, 1, 3)),
        ("gemini", gemini.usage.expect("Gemini usage"), (2, 1, 3)),
    ];
    for (name, new_usage, legacy_usage) in cases {
        ...
        assert_eq!(new_tuple, legacy_usage, "legacy/new token usage: {name}");
        let semantic = semantic_usage_from_token_usage(&new_usage);
        assert_eq!(semantic.input_tokens, legacy_usage.0, "semantic input: {name}");
        assert_eq!(semantic.output_tokens, legacy_usage.1, "semantic output: {name}");
        assert_eq!(semantic.total_tokens, legacy_usage.2, "semantic total: {name}");
        assert_eq!(semantic.cache.read_input_tokens, 0, "semantic cache: {name}");
...
fn stream_conformance_preserves_order_unknown_events_and_cancel_class() {
    ...
    let chat_text = chat_events.iter().filter_map(|event| match event {
        lmm_contracts::relay::CanonicalStreamEvent::TextDelta { delta, .. } => Some(delta.as_str()),
        _ => None,
    }).collect::<Vec<_>>();
    assert_eq!(chat_text, vec!["a", "b"]);
    assert!(chat_events.iter().any(|event| matches!(event, lmm_contracts::relay::CanonicalStreamEvent::ResponseEnd { finish_reason: FinishReason::Stop, .. })));

    let responses: ResponsesStreamSnapshot = serde_json::from_str(RESPONSES_STREAM).expect("Responses stream");
    ...
    assert!(responses_events.iter().any(|event| matches!(event, lmm_contracts::relay::CanonicalStreamEvent::ResponseEnd { finish_reason: FinishReason::Stop, .. })));

    let gemini: GeminiStreamSnapshot = serde_json::from_str(GEMINI_STREAM).expect("Gemini stream");
    let gemini = gemini_stream_to_canonical(&gemini, "gemini-2.5-pro").expect("Gemini stream conversion");
    assert!(gemini.value.output.iter().any(|part| matches!(part, CanonicalContent::Text { text } if text == "ab")));
    ...
    let claude_events = claude_stream_to_semantic_events(&claude).expect("Claude stream conversion");
    ...
    assert!(claude_events.iter().any(|event| matches!(event, ClaudeStreamSemanticEvent::Ping)));
    assert!(claude_events.iter().any(|event| matches!(event, ClaudeStreamSemanticEvent::MessageStop)));
...
fn sse_regression_corpus_is_incremental_bounded_and_panic_free() {
    ...
    for byte in b"event: future\ndata: {\"x\":1}\n\n" {
        frames.extend(parser.feed(std::slice::from_ref(byte)).expect("valid byte feed"));
    }
    ...
    let done = parse_sse_frames(b"data: [DONE]\n\n", DEFAULT_MAX_FRAME_BYTES)
        .expect("terminal SSE frame");
    assert!(done[0].is_done());
    assert!(parse_sse_frames(b"data: unfinished\n", 64)
        .expect("strict EOF")
        .is_empty());
    assert_eq!(
...
fn arbitrary_sse_bytes_are_bounded_without_panics() {
    for width in 1..=7 {
        ...
        assert!(result.is_ok(), "arbitrary SSE bytes panicked at width {width}");
    }
}
...
fn observer_and_parser_are_race_ready_with_per_thread_parser_state() {
    ...
    for _worker in 0..4 {
        ...
        handles.push(thread::spawn(move || {
            let labels = MetricLabels::native_raw(Protocol::OpenAi, true, ConversionResult::Success);
            for _ in 0..64 {
...
fn ordered_ir_keeps_authentic_state_and_requires_structured_loss_for_mutation() {
    ...
    let mut message = Item::new(ItemKind::Message, Role::User, Provenance::new(Protocol::OpenAiResponses));
    message.push_part(Part::text("first"));
    message.push_part(Part::opaque(OpaqueProviderState::authentic_gemini_thought_signature(
        "sig-authentic".to_owned(),
        Some("gemini-2.5-pro".to_owned()),
    )));
    envelope.push_item(message).expect("valid ordered message");
    assert_eq!(envelope.ordered_items()[0].ordered_parts()[0].text.as_deref(), Some("first"));
    assert!(envelope.validate_exact_round_trip().is_ok());

    let mut second = Item::new(ItemKind::Message, Role::User, Provenance::new(Protocol::OpenAiResponses));
    ...
    assert_eq!(envelope.ordered_items()[0].ordered_parts()[0].text.as_deref(), Some("second"));
...
fn ordered_ir_tool_links_keep_item_id_distinct_from_call_id() {
    ...
    let result = Item::tool_result(
        OpaqueId::authentic("call-authentic", Protocol::OpenAi),
        vec![Part::function(FunctionData::result(None, JsonData::String("ok".to_owned())))],
        Provenance::new(Protocol::OpenAi),
    );
    envelope.push_item(call).expect("tool call");
    envelope.push_item(result).expect("linked tool result");
    assert_eq!(envelope.ordered_items()[0].id.as_ref().map(OpaqueId::as_str), Some("item-id"));
    assert_eq!(envelope.ordered_items()[0].call_id.as_ref().map(OpaqueId::as_str), Some("call-authentic"));
    assert_eq!(envelope.ordered_items()[1].call_id.as_ref().map(OpaqueId::as_str), Some("call-authentic"));
    ...
    let mut synthetic_item = Item::new(ItemKind::Message, Role::User, Provenance::new(Protocol::OpenAi));
    synthetic_item.id = Some(synthetic);
    let mut synthetic_envelope = Envelope::new(Protocol::OpenAi, "gpt-test");
    synthetic_envelope.push_item(synthetic_item).expect("synthetic id is structurally valid");
...
fn usage_cache_billing_and_extensions_round_trip_without_float_loss() {
    ...
    envelope.extensions.insert("future_usage_field".to_owned(), JsonData::Bool(true));
...
fn ownership_gate_defaults_closed_and_requires_all_five_differentials() {
    ...
    below_canary.set_canary_basis_points(99).expect("bounded canary");
    assert!(matches!(
...
fn shadow_comparison_is_body_free_and_client_abort_is_bounded() {
    let runner = ShadowRunner::new(
        |_request: &LocalRequest<'_>| Ok::<LocalConversionSummary, LocalConversionError>(local_summary("old", 7)),
        |_request: &LocalRequest<'_>| Ok::<LocalConversionSummary, LocalConversionError>(local_summary("old", 7)),
        Protocol::OpenAi,
    ...
    let mismatch = ShadowRunner::new(
        |_request: &LocalRequest<'_>| Ok::<LocalConversionSummary, LocalConversionError>(local_summary("old", 7)),
        |_request: &LocalRequest<'_>| Ok::<LocalConversionSummary, LocalConversionError>(local_summary("new", 7)),
        Protocol::OpenAi,
        Protocol::OpenAiResponses,
        true,
    )
    .compare(&LocalRequest::new(b"private request body"));
    assert!(mismatch.differences.contains(&ShadowDifference::ConverterId));
