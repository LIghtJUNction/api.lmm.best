...
rust_binary=${LMM_MODELS_RUST_BINARY:-"$repo_root/rust/target/debug/lmm-api-rs"}
...
rust_build_input_manifest() {
  ...
  local -a roots=(
    "$repo_root/rust/apps/lmm-api-rs"
    "$repo_root/rust/crates/application"
    "$repo_root/rust/crates/contracts"
    "$repo_root/rust/crates/domain"
    "$repo_root/rust/crates/observability"
  )
  {
    for input in "$repo_root/rust/Cargo.toml" "$repo_root/rust/Cargo.lock"; do
      [[ -f $input ]] && printf '%s\0' "$input"
    done
    for input in "$repo_root/rust/.cargo" "$repo_root/rust/rust-toolchain.toml" "$repo_root/rust/rust-toolchain"; do
...
if [[ $probe_only == 1 ]]; then
  ...
  jq -cn --arg go_content_sha256 "$frozen_go_manifest_sha256" --arg rust_build_input_manifest_sha256 "$rust_build_input_manifest_sha256" \
    ...
    '{test:"models-listener-differential",mode:"probe",frozen_go_content_sha256:$go_content_sha256,rust_build_input_manifest_sha256:$rust_build_input_manifest_sha256,rust_binary_sha256:(if $rust_binary_sha256 == "null" then null else $rust_binary_sha256 end),build_input_roots:["rust/Cargo.toml","rust/Cargo.lock","rust/apps/lmm-api-rs","rust/crates/application","rust/crates/contracts","rust/crates/domain","rust/crates/observability"],result:"passed"}'
  exit 0
fi
