...
write_rust_build_manifest() {
  ...
  (
    ...
    for directory in rust/apps/lmm-api-rs/src rust/apps/lmm-api-rs/assets rust/crates; do
...
start_rust() {
  ...
  (
    ...
    export DATABASE_URL VALKEY_URL SESSION_SECRET CRYPTO_SECRET PASSWORD_LOGIN_ENABLED \
      ...
      CRITICAL_RATE_LIMIT_ENABLE CRITICAL_RATE_LIMIT CRITICAL_RATE_LIMIT_DURATION \
      SEARCH_RATE_LIMIT_ENABLE SEARCH_RATE_LIMIT SEARCH_RATE_LIMIT_DURATION
    LMM_RS_TEST_INSTANCE=1 LMM_RS_LISTEN_ADDR="127.0.0.1:$rust_port" LMM_RS_SLOT=single LMM_SCHEMA_CONTRACT=1 VERSION=v0.0.0 exec "$repo_root/rust/target/debug/lmm-api-rs"
...
"$repo_root/rust/scripts/capture-legacy-route-contract.sh" --base-url "http://127.0.0.1:$go_port" --request "$runtime/requests/api-token-anonymous.json" --output "$runtime/fixtures/api-token-anonymous.json" --postgres-url "$go_dsn"
GO_BASE_URL="http://127.0.0.1:$go_port" RUST_BASE_URL="http://127.0.0.1:$rust_port" \
  ...
  "$repo_root/rust/scripts/run-route-differential.sh" "$runtime/fixtures"
scenario_total=$((scenario_total + 1))
