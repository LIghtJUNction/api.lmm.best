...
fixtures=${1:-"$repo_root/rust/behavior-oracle/tests/missing-routes-transaction-fixtures.json"}
matrix=${MISSING_ROUTES_MATRIX:-"$repo_root/rust/behavior-oracle/tests/missing-routes-matrix.tsv"}
...
jq -cn \
  ...
  '{test:"missing-routes-transaction-fixtures",fixtures:$routes,phases:$phases,isolated:true,production_access:false,result:"contract-passed",approval_credit:false}'

if ! $contract_only; then
  exec "$repo_root/rust/behavior-oracle/tests/missing-routes-transaction-differential.sh"
fi
