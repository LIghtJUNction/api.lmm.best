...
legacy=${DRAFT_BASELINE_PATH:-"$repo_root/rust/routes/legacy-go-routes.tsv"}
outside_allowlist=${DRAFT_OUTSIDE_BASELINE_ALLOWLIST-"$repo_root/rust/routes/draft-route-completion-allowlist.tsv"}
legacy_stub_ledger=${DRAFT_LEGACY_EQUIVALENT_STUB_LEDGER-"$repo_root/rust/routes/legacy-equivalent-stubs.tsv"}
...
require_approved_legacy_stubs() {
  ...
  while IFS=$'\t' read -r method path rust_source handler frozen_ledger behavior_test rationale extra; do
    ...
    [[ $rust_source == rust/*.rs && -f "$repo_root/$rust_source" && $behavior_test == rust/*.rs && -f "$repo_root/$behavior_test" ]] || {
