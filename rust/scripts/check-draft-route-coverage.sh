...
router_root=${DRAFT_ROUTER_ROOT:-"$repo_root/rust/apps/lmm-api-rs/src"}
baseline=${DRAFT_BASELINE_PATH:-"$repo_root/rust/routes/legacy-go-routes.tsv"}
gate=${DRAFT_GATE_PATH:-"$repo_root/rust/routes/migration-gate.tsv"}
plan=${DRAFT_PLAN_PATH:-"$repo_root/rust/routes/migration-plan.tsv"}
...
outside_allowlist=${DRAFT_OUTSIDE_BASELINE_ALLOWLIST-"$repo_root/rust/routes/draft-route-completion-allowlist.tsv"}
    ...
    if (!defined $allowlist_header) {
        ...
        while (my $line = <$allowlist_handle>) {
            ...
            if ($method !~ /^(?:GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|TRACE|CONNECT)$/ || !defined $normalized
                ...
                || $source_path !~ m{^rust/[A-Za-z0-9_./-]+\.rs$}
                || $source_path =~ m{(?:^|/)\.\.?/}
