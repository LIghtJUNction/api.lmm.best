...
legacy="$repo_root/rust/routes/legacy-go-routes.tsv"
plan="${MIGRATION_PLAN_PATH:-$repo_root/rust/routes/migration-plan.tsv}"
gate="${MIGRATION_GATE_PATH:-$repo_root/rust/routes/migration-gate.tsv}"
review="${MIGRATION_INTEGRATION_REVIEW_PATH:-$repo_root/rust/routes/integration-review.tsv}"
...
candidate_dir="$repo_root/rust/apps/lmm-api-rs/src/migration_routes"
mapfile -t candidate_files < <(rg --files "$candidate_dir")
candidate_count=${#candidate_files[@]}
candidate_mod="$repo_root/rust/apps/lmm-api-rs/src/migration_routes.rs"
...
root_router="${MIGRATION_ROOT_ROUTER_PATH:-$repo_root/rust/apps/lmm-api-rs/src/http.rs}"
